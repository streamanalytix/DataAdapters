...
public final class SftpUtilChannel {
     ...
     *
     * @param userName            for SFTP connection
     * @param password            for SFTP connection
     * @param sftpHost            for SFTP connection
     * @param location            for SFTP connection
     * @param fileType            is type of file we are going handle
     * @param context            is the context of Spark
     * @param delayBtwConnectionRetires            is basically max time for making a connection to SFTP
     * @param connectionRetries            is number of times we can re-try for connection in case of time-out or any other network latency error
     * @return Dataframe we will return data frame after reading from SFTP
     * @throws FileSystemException the file system exception
     */
    public static DataFrame createDataFrameFromSftp(String userName, String password, String sftpHost, String location, String fileType,
        ...
        try {
            ...
            switch (fileType) {
                ...
                case SftpConstant.Values.AVRO:
                    dataFrame = sqlContext.read().format("com.databricks.spark.avro").load(SftpConstant.Values.HDFS_CREATE_DF_PATH);
                    break;
                case SftpConstant.Values.PARQUET:
                    dataFrame = sqlContext.read().parquet(SftpConstant.Values.HDFS_CREATE_DF_PATH);
                    break;
                case SftpConstant.Values.JSON:
                    dataFrame = sqlContext.read().json(SftpConstant.Values.HDFS_CREATE_DF_PATH);
                    break;
                default:
     ...
     * @param sftpHost            for SFTP connection
     * @param location            for SFTP connection
     * @param fileType            is type of file we will handle
     * @param manager            is FileSystem manager
     * @param delayBtwConnectionRetires            is basically max time for making a connection to SFTP
     * @param connectionRetries            is number of times we can re-try for connection in case of time-out or any other network latency error
