...
public final class SftpUtilEmitter {
    ...
    public static void writeToSftp(String userName, String password, String sftpHost, String location, String fileType, DataFrame dataFrame,
        ...
        try {
            ...
            // 2. We need to read file again with single schema , in copyMerge(HDFS operation) we were getting schema / header scrapped
                case SftpConstant.Values.CSV:
                    dataFrame.coalesce(1).write().option("header", "true").option("inferSchema", "true").format("com.databricks.spark.csv")
                            .save(SftpConstant.Values.HDFS_WRITE_DF_PATH);
                    break;
                case SftpConstant.Values.AVRO:
                    dataFrame.coalesce(1).write().format("com.databricks.spark.avro").save(SftpConstant.Values.HDFS_WRITE_DF_PATH);
                    break;
