    ...
    public void process(JSONObject json, Map<String, Object> configMap) throws Exception {
        ...
        if (shardKey != null && !("".equals(shardKey.trim()))) {
            try {
                ...
                LOGGER.error("Not able to process sharding due to some exception.** Please check ", ex);
            }
        }
        ...
        for (String key : outPutColumns) {
            ...
            if (dateformat != null) {
                try {
                    ...
                    LOGGER.error("Failed is not a date type ",e);
                }
                ...
                if (json.get(key) instanceof String) {
                    ...
                    if (json.get(key) instanceof Long) {
                        try {
                            ...
                            LOGGER.error("value is long so need to convert to int" + "but in exception class  " + ex + (json.get(key)));
                            basicDbObj.put(key, json.get(key));
                        }
                    } else {
                        LOGGER.info("not long value  but not string" + "  " + (json.get(key)));
                        basicDbObj.put(key, json.get(key));
                    }
                }
